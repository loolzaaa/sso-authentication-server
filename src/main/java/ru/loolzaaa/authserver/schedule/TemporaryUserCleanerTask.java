        ...
        for (User u : users) {
            ...
            if (!userPrincipal.isAccountNonExpired()) {
                try {
                    ...
                    deleteUserCounter++;
                    log.info("Temporary user [{}] deleted: {}", u.getLogin(), result.getText());
                } catch (RequestErrorException e) {
                    log.error("Failed to delete temporary user [{}]: {}", u.getLogin(), e.getMessage());
                } catch (Exception e) {
                    log.error("Unexpected error while deleting temporary user [{}]", u.getLogin(), e);
                }
            }
        }
